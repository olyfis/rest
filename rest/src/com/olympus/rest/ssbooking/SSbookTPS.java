...
import com.olympus.rest.SapObj;
import com.olympus.rest.DButil;


		@WebServlet("/ssbooktps")
		public class SSbookTPS extends HttpServlet {
			private final Logger logger = Logger.getLogger(SSbookTPS.class.getName()); // define logger
						...
						if (poNumber.length() > 15) {
							...
							if (poNumber.startsWith("101") || poNumber.startsWith("102")) {
								if (poNumber.length() < 15) {
									...
									String[] parts = poNumber.split("-");
									if (parts[1].length() != 7) {
										System.out.println("**7** POError-> not 7:" + parts[1] + "-- PO=" + poNumber + "--");
										continue;
									}
									
									if (parts[2].length() != 3) {
										System.out.println("**3** POError-> not 3:" + parts[2] + "-- PO=" + poNumber + "--");
										continue;
									}
